...
std::string BTagEntry::makeNewCSVHeader()
{
  return "wp,"
         "type,"
         "syst,"
...
std::string BTagEntry::makeNewCSVLine() const
{
  std::stringstream buff;
  buff << getWP(params.operatingPoint) 
...
std::string BTagEntry::getWP(OperatingPoint op) const
{
  if (params.operatingPoint==0) return "L";
  if (params.operatingPoint==1) return "M";
  if (params.operatingPoint==2) return "T";
  if (params.operatingPoint==3) return "XT";
  if (params.operatingPoint==4) return "XXT";
  if (params.operatingPoint==5) return "-";
  return "?";
}
...
void BTagCalibration::makeNewCSV(std::ostream &s) const
{
  //s << tagger_ << ";" << BTagEntry::makeNewCSVHeader();
  s << BTagEntry::makeNewCSVHeader();
...
std::string BTagCalibration::makeNewCSV() const
{
  std::stringstream buff;
  makeNewCSV(buff);
  return buff.str();
